...
# Konfigurasi logging buat output bersih
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

#Buat variabel dari file .env
load_dotenv()
init(autoreset=True)
...
def get_spotify_client():
    """Menginisialisasi dan mengembalikan klien Spotify.."""
    client_id = os.getenv("SPOTIFY_CLIENT-ID")
    client_secret = os.getenv("SPOTIFY_CLIENT-SEC")
    if not client_id or not client_secret:
        logging.error("Yahhh, Credentials Spotify (SPOTIFY_CLIENT-ID, SPOTIFY_CLIENT-SEC) ga ditemuin:(")
        logging.info("Pastiin lagi kalo kamu udah buat file .env dan udah diisi.")
        exit()
    ...
    try:
        ...
        logging.error(f"Gagal mautentikasi dengan spotify nihh: {e}")
        exit()

def get_track_info(sp, url):
    """Ambil metadata track dari URL Spotify (Track, Album. atau Playlist)."""
    track_list = []
    album_info = None
    ...
    try:
        if "track" in url:
            ...
            logging.warning(f"URL ga didukung atau ga valid. {url}")
            return []
    except spotipy.exceptions.SpotifyException as e:
        logging.error(f"Error saat ambil data dari Spotify, Maaf yaa. {e}")
        return []
...
╔════════════════════════════════════════════════════════════╗
║                                                          ║
║   {Fore.YELLOW}Spotify Downloader CLI by Affandi Anugrah{Fore.MAGENTA}         ║
║                                                          ║
╚════════════════════════════════════════════════════════════╝
{Style.RESET_ALL}
"""
